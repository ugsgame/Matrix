...
abstract public class ShapeManager implements Manager {
	...
	// 鼠标状态
	private final static int MOUSE_STATE_ROATE = 5;
	...
	abstract void childMousePressed(MouseEvent e);
	...
	public int getShapePos(Shape shape){
	...
	public ArrayList getRealShapes() {
		return shapes;
	}
	...
	private void selfMouseDragged(MouseEvent e) {
		...
		mouseY = getMouseY(e);
		// checkMouseXY();
		if(mouseState == MOUSE_STATE_ROATE)
			mouseDraggedR(e);
		else if (!scrollablePanel.isMouseMovingViewport()) {
			mouseDragged(e);
		}
	...
	protected void mouseDraggedR(MouseEvent e) 
	{
		switch (mouseState) {
		case MOUSE_STATE_ROATE:
			this.selection.roate(mouseY - oldMouseY ,oldMouseX);
			oldMouseY = mouseY;
	...
	public void Rpressed(KeyEvent e)
	{
		selection.Rpressed(e);
		this.rpress = true;
	}
	...
	public void Rrealse(KeyEvent e)
	{
		this.setMouseState(MOUSE_STATE_NORMAL);
		this.selection.Rrealse(e);
		this.rpress = false;
	}
	...
	protected Shape getTopShapeAtPoint(int x, int y) {
		...
		for (int i = shapes.size() - 1; i >= 0; --i) {
			shape = (Shape) (shapes.get(i));
			if (shape.containPoint(x, y) && shape.getLayer() == layer) { return shape; }
		}
		return null;
	}
	...
	protected void multiSelectionMouseLeftPressed(MouseEvent e) {
		if (selection.pointInShapes(e.getX(), e.getY())) { // 按键点在已选择的物体内
			if(this.rpress)
			{
				setMouseState(MOUSE_STATE_ROATE);
			}
			else if (e.isControlDown()) { // 按了Ctrl键
	...
	abstract public void paintShapesOther(Graphics g);
}
...
//		paintFore(g);
	...
	private void paintFore(Graphics g, int layer) {
		// TODO Auto-generated method stub
		for (int i = 0; i < foreManagers.size(); ++i) {
			((Manager) (foreManagers.get(i))).paintStatic(g, layer);
		}
	}

	private void paintBack(Graphics g, int layer) {
		// TODO Auto-generated method stub
		for (int i = 0; i < backManagers.size(); ++i) {
			((Manager) (backManagers.get(i))).paintStatic(g, layer);
		}
	}
