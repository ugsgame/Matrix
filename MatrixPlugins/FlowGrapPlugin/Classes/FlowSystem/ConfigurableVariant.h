...
namespace NConfigurableVariantHelpers
{
	...
	struct VTableImpl< T, false, IVisitorClass, IMutatingVisitorClass >
	{
		...
		static inline void Construct( void * ptr, const T& value )
		{
			new (ptr) T(value);
		}
		...
		static inline const T* Get( const void * ptr )
		{
		...
		static inline void Visit( IVisitorClass * pVisitor, const void * pSrc, void * pImpl )
		{
			((SAbstractVisitDeclarator<const T&>*)pVisitor)->AbstractVisit( *static_cast<const T*>(pSrc), pImpl );
		}
		static inline void MutatingVisit( IMutatingVisitorClass * pVisitor, void * pSrc, void * pImpl )
		{
			((SAbstractVisitDeclarator<T&>*)pVisitor)->AbstractVisit( *static_cast<T*>(pSrc), pImpl );
		}
// 		static inline void GetMemoryStatistics(ICrySizer * s, const void *ptr)
// 		{
// 			s->AddObject(*static_cast<T*>(const_cast<void*>(ptr)));
// 		}
		static inline int  GetMemorySize(const void *)
		{
			return 0;
		}
	};
...
// 		static inline void GetMemoryStatistics(ICrySizer * s, const void * p)
// 		{
// 			s->Add(**static_cast<const T*const*>(p));
// 		}
		static inline int GetMemorySize(const void * p)
		{
	...
	public:
		...
		static inline bool f( int tag, Callback& cb )
		{
			if (tag == i)
			{
				cb( H() );
	...
	public:
		...
		template <class From>
		inline void Visit( const From& value )
		{
	...
	public:
		template <class V>
		inline static bool func( const V& from, V& to )
		{
			if (from.GetTag() == SLOT)
			{
	...
	{
	public:
		static inline bool ConvertValue( const From& from, To& to )
		{
			return false;
		}
	};
	...
	{
	public:
		static inline bool ConvertValue( const From& from, From& to )
		{
			to = from;
			return true;
		}
	...
	{
	public:
		static inline T ConstructedValue()
		{
			return T();
		}
	};
/*
	...
	{
	public:
		static inline Vec3 ConstructedValue()
		{
			return Vec3(ZERO);
		}
	};
...
class CConfigurableVariant
{
	...
	static const int FLAG_ISLOCKED = 1;
	// and the other we allow the user of this class to decide the semantics
	static const int FLAG_USER = 2;
	static const int FLAGS_MASK = FLAG_ISLOCKED | FLAG_USER;
...
// 	}
	...
	inline bool IsUserFlagSet() const
	{
		return (m_VTableAndFlags & FLAG_USER) != 0;
	}
	inline void SetUserFlag( bool value )
	{
		if (value)
	...
	inline void ToggleUserFlag()
	{
		m_VTableAndFlags ^= FLAG_USER;
	}
	inline void ClearUserFlag()
	{
		m_VTableAndFlags &= ~FLAG_USER;
	}
...
private:
	...
	// type and value
	template <class T>
	inline void Construct( const T& value, bool isLocked )
	{
		using namespace NConfigurableVariantHelpers;
		m_VTableAndFlags = 0;
	...
	template <class T>
	inline VTable * GetVTableForType()
	{
		using namespace NConfigurableVariantHelpers;
	...
	// pointer also
	inline const VTable * GetVTable() const
	{
	...
	// pointer also
	inline void SetVTable( const VTable * pVTable )
	{
		m_VTableAndFlags = 
