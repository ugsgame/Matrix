...
public:
	...
	/// </example>
	virtual IMonoClass *GetClass(const char *className, const char *nameSpace = "Matrix") = 0;
	...
	/// </summary>
	virtual IMonoException *GetException(const char *nameSpace, const char *exceptionClass, const char *message = NULL, ...)
	{
		va_list	args;
		char szBuffer[4096];
		va_start(args, message);
#ifdef WIN32
		int count = vsnprintf_s(szBuffer, sizeof(szBuffer),4096, message, args);
#else
		int count = vsnprintf(szBuffer, sizeof(szBuffer), message, args);
#endif
