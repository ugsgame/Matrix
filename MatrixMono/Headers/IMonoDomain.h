...
struct IMonoDomain
{
	...
	/// <summary>
	///	Loads an assembly into the domain by image
	/// </summary>
	virtual IMonoAssembly *LoadAssembly(void* data, int size , const char* path, void* mdbData = NULL , int mdbSize = 0) = 0;

	/// <summary>
	/// Loads an assembly into the domain.
	/// </summary>
	virtual IMonoAssembly *LoadAssembly(const char *file, void* mdbData = NULL ,int mdbSize = 0,bool shadowCopy = false, bool convertPdbToMdb = true) = 0;
	...
	/// Vec3[] myElements;
	/// </example>
	virtual IMonoArray *CreateArray(int size, IMonoClass *pElementClass = 0, bool allowGC = true) = 0;

	/// <summary>
	/// Creates a dynamically resizing mono array.
	/// if element is null, uses object class.
	/// </summary>
	virtual IMonoArray *CreateDynamicArray(IMonoClass *pElementClass = 0, int size = 0, bool allowGC = true) = 0;
...
{
	return ShareMonoScriptSystem()->GetActiveDomain()->CreateArray(numArgs, pElementClass);
}

/// <summary>
/// Creates an IDynMonoArray with the specified size.
/// </summary>
static IMonoArray *CreateDynamicMonoArray(IMonoClass *pElementClass = NULL, int size = 0)
{
	return ShareMonoScriptSystem()->GetActiveDomain()->CreateDynamicArray(pElementClass, size);
}
...
{
	return ShareMonoScriptSystem()->GetActiveDomain()->CreateMonoString(cryString);
}
